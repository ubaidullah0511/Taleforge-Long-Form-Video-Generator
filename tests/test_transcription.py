"""Offline tests for app.transcription — Groq's Whisper endpoint is mocked
throughout (no real audio file, no network/API keys needed).
...
def test_get_narration_timing_parses_word_level_timestamps():
    ...
    try:
        with patch("app.transcription._transcribe_audio_raw", return_value=_mock_groq_response(words)):
...
def test_get_narration_timing_raises_when_groq_call_fails():
    with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as f:
        f.write(b"fake audio bytes")
        audio_path = f.name

    try:
        with patch("app.transcription._transcribe_audio_raw", side_effect=RuntimeError("network down")):
            try:
...
def test_get_narration_timing_raises_when_response_has_no_words():
    ...
    try:
        with patch("app.transcription._transcribe_audio_raw", return_value={"text": "hello", "segments": []}):
            try:
                get_narration_timing(audio_path)
                raise AssertionError("expected TranscriptionError")
            except TranscriptionError as exc:
...
if __name__ == "__main__":
    ...
    test_get_narration_timing_raises_when_groq_call_fails()
